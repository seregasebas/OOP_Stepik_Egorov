...
#     def tell_about_yourself(self):
#         Surgeon.tell_about_yourself(self)
...
#             res.append(f'{song.title} - {song.artist}')
#         return str(res)
#
#
# playlist = Playlist()
...
#
#     def __init__(self, *args):
#         # self.values = sorted([arg for arg in args if type(arg) == int])
#         self.values = []
#         if len(args) > 0:
#             for arg in args:
...
#         else:
#             return f"Вектор{tuple(val for val in self.values)}"
#
#     def __add__(self, other):
#         if isinstance(other, int):
#             res = [arg + other for arg in self.values]
#             return Vector(*res)
#         if isinstance(other, Vector) and len(self.values) == len(other.values):
#             res = [se + ot for se, ot in zip(self.values, other.values)]
#             return Vector(*res)
...
#
#     def __str__(self):
#         igr = '\n'.join(map(str, sorted(self.ingredients, key = lambda x: x.weight, reverse = True)))
...
#         if not any((ch.isupper() for ch in password) and (ch.islower() for ch in password)):
...
#         with open('txt/easy_passwords.txt', 'r', encoding='utf-8') as f:
...
#             raise TypeError
#         if '@' not in login and login.count('@') !=1:
#             raise ValueError
#         else:
...
#                 and not Registration.is_include_all_register(password)
#                 and not Registration.is_include_only_latin(password)):
...
#     @classmethod
#     def margherita(cls):
#         return cls(ingredients=['mozzarella', 'tomatoes'])
#     @classmethod
#     def peperoni(cls):
#         return cls(ingredients=['mozzarella', 'peperoni', 'tomatoes'])
#     @classmethod
#     def barbecue(cls):
#         return cls(ingredients=['mozzarella', 'red onion', 'sauce bbq', 'chicken'])
# bbq = Pizza.barbecue()
...
#     @property
#     def notes_list(self):
#         for ind, note in enumerate(self._notes, start=1):
#             print(f'{ind}.{note}')
#
